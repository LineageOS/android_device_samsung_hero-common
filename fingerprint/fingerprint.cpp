...
static int fingerprint_enroll(struct fingerprint_device __unused *dev, const hw_auth_token_t *hat, uint32_t gid, uint32_t timeout_sec)
{
...
static int fingerprint_set_active_group(struct fingerprint_device __unused *dev, uint32_t gid, const char *store_path)
{
    return (*bauth_handle->ss_fingerprint_set_active_group)(gid, store_path);
}

static int fingerprint_authenticate(struct fingerprint_device __unused *dev, uint64_t operation_id, uint32_t gid)
{
